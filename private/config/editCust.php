        ...
        <div class="carda card">
          ...
          <div class="card-body">
            <form action="" method="post">
              <?php foreach ($member as $items) : ?>
                <input type="hidden" name="idcust" id="" value="<?= $items['id_member'] ?>">
                <div class="mb-3">
                ...
                <div class="mb-3">
                  ...
                  <select name="kelamin" id="kelamin" class="form-select" required>
                    ...
                    <?php
                    if ($items['kelamin'] == 'L') {
                      $items['kelamin'] = 'Laki-Laki';
                    } elseif ($items['kelamin'] == 'P') {
                      $items['kelamin'] = 'Perempuan';
                    }
                    ?>
                    <option selected hidden value="<?= $items['kelamin'] ?>"><?= $items['kelamin'] ?></option>
          ...
          <div class="card-footer text-muted">
            <button type="submit" class="btn btn-success float-end" name="uptMember">Kirim</button>
          </div>
          </form>
        </div>
      </div>
            ...
            <tbody>
              <tr class="">
                ...
                foreach ($member as $items) : ?>
                  <td scope="row"><?= $no++ ?>.</td>
                  <td><?= $items['nama'] ?><img src="../assets/img/<?= $items['kelamin'] == 'L' ? 'men-line.png' : 'women-line.png' ?>" width="13px" alt="Gender Beliau"></td>
                  <td><?= $items['alamat'] ?></td>
                  <td><?= $items['tlp'] ?></td>
                  <td class="text-center">
                    <a href="../config/editCust?cust=<?= $items['id_member'] ?>" class="badge btn text-dark"><i class="ri-edit-line"></i></a>
                    <a href="../config/delCust?cust=<?= $items['id_member'] ?>" onclick="return confirm('Yakin untuk hapus?')" class="badge btn text-dark"><i class="ri-delete-bin-line"></i></a>
                  </td>
              </tr>
            <?php endforeach; ?>
            </tbody>
